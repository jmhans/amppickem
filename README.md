...
Requires a `.env.local` with `POSTGRES_URL`, `POSTGRES_URL_DEV`, `AUTH0_SECRET`, `APP_BASE_URL`, `AUTH0_DOMAIN`, `AUTH0_CLIENT_ID`, `AUTH0_CLIENT_SECRET`, `CRON_SECRET` — see amp-playoff-fantasy for the shared Auth0/DB values. For the commissioner-email feature, also `GMAIL_USER`, `GMAIL_APP_PASSWORD`, `COMMISSIONER_EMAIL` (see "Commissioner sync" below); for pick reminders, `NEXT_PUBLIC_VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT` (see "Pick reminders" below); for AI-drafted recaps, `GEMINI_API_KEY` (a free Google AI Studio key — see "Weekly Recaps" below).
